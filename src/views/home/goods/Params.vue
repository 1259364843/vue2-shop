  ...
  methods: {
    ...
    addParams() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return
        // 发送添加参数网络请求
        const res = await addCateParam(this.cateId, {
    ...
    async showEditDialog(id) {
      const res = await getParamById(this.cateId, id, {
        attr_sel: this.activeName
      })
      if (res.meta.status !== 200) {
    ...
    editParams() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        //   }
        // )
        const res = await updateParamById(this.cateId, this.editForm.attr_id, {
          attr_name: this.editForm.attr_name,
          attr_sel: this.activeName
        })
        if (res.meta.status !== 200) {
    ...
    async removeParams(id) {
      ...
      // )
      const res = await deleteParamById(this.cateId, id)
      if (res.meta.status !== 200) {
    ...
    async saveAttrVals(row) {
      const res = await updateParamById(this.cateId, row.attr_id, {
