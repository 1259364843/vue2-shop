  ...
  methods: {
    ...
    addRolesUser() {
      this.$refs.addRolesForm.validate(async (valid) => {
        if (!valid) return
        const res = await addUserRoles(this.addRolesForm)
    ...
    editFormInfo() {
      this.$refs.editFormRef.validate((valid) => {
        if (!valid) return
        setRoleInfo(this.editRolesForm.roleId, {
    ...
    async removeRightById(roles, id) {
      ...
      // 发送网络请求
      const res = await removeRolePermission(roles.id, id)
      if (res.meta.status !== 200) {
    ...
    async allotRights() {
      ...
      // 发送网络请求
      const res = await grantRolePermission(this.rolesId, idStr)
      if (res.meta.status !== 200) {
